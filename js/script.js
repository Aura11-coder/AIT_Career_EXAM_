...
async function handleFormSubmit(validators, errorMessages, submitBtn) {
  ...
  try {
    ...
    let emailCheck;
    try {
      emailCheck = await apiCheckEmail(candidateData.email);
    } catch (apiErr) {
      // If API is not configured, allow proceeding (dev/demo mode)
      if (AIT_CONFIG.SCRIPT_URL.includes('YOUR_SCRIPT_ID_HERE')) {
    ...
    try {
      await apiRegisterCandidate(candidateData);
    } catch (regErr) {
      // Non-fatal: continue even if registration API fails
      if (!AIT_CONFIG.SCRIPT_URL.includes('YOUR_SCRIPT_ID_HERE')) {
        console.warn('Registration API error (non-fatal):', regErr);
      }
    }
